...
def parse_drop_target(mode_str: str, direction_str: str, box_w: float, box_h: float, box_d: float) -> np.ndarray:
    """
    [WHTOOLS] 낙하 모드와 방향 문자열을 파싱하여 바닥에 닿을 로컈 좌표(타겟 점)을 반환합니다.
    
    Args:
        mode_str  (str):   낙하 모드 ("PARCEL", "LTL", "GENERAL")
        direction_str (str): 방향 문자열 ("Face 3", "Edge 3-4", "Corner 2-3-5", "front-bottom-left" 등)
        box_w (float): 박스 너비의 절반 (m)
        box_h (float): 박스 높이의 절반 (m)
        box_d (float): 박스 깊이의 절반 (m)
    
    Returns:
        np.ndarray: 로컈 좌표계의 바닥 충돌 외곽점 [x, y, z] (m)
    
    Notes:
        ISTA PARCEL 면 번호 맵핑 (Y=Height, X=Width, Z=Depth 기준):
          1: Top (+Y), 2: Bottom (-Y), 3: Right (+X), 4: Left (-X), 5: Front (+Z), 6: Back (-Z)
        ISTA LTL 면 번호 맵핑:
          1: Top (+Y), 2: Back (-Z), 3: Bottom (-Y), 4: Front (+Z), 5: Right (+X), 6: Left (-X)
        
        평행 면 조합(모서리 불가):
          PARCEL: 1-2(Top/Bottom), 3-4(Right/Left), 5-6(Front/Back)
          LTL:    1-3(Top/Bottom), 2-4(Back/Front), 5-6(Right/Left)
    """
    import re
    ...
    # [WHTOOLS] 평행 면 조합 목록 - 서로 마주보는 면들은 모서리를 형성할 수 없음
    # frozenset으로 저장하여 순서에 관계없이 매칭
    _parcel_parallel_pairs = {
        frozenset([1, 2]),  # Top/Bottom - 평행
        frozenset([3, 4]),  # Right/Left - 평행
        frozenset([5, 6]),  # Front/Back - 평행
    }
    _ltl_parallel_pairs = {
        frozenset([1, 3]),  # Top/Bottom - 평행
        frozenset([2, 4]),  # Back/Front - 평행
        frozenset([5, 6]),  # Right/Left - 평행
    }
    
    face_map = ltl_map if mode == 'LTL' else parcel_map
    parallel_pairs = _ltl_parallel_pairs if mode == 'LTL' else _parcel_parallel_pairs
    vec = np.array([0.0, 0.0, 0.0])
    
    # 숫자(1~6)가 포함된 경우 해당 맵핑 적용
    nums = [int(n) for n in re.findall(r'[1-6]', direct)]
    if nums:
        # [WHTOOLS BUG-FIX] 평행 면 조합 사전 검출 - 모서리 불가능한 조합을 미리 경고
        if len(nums) == 2:
            pair_key = frozenset(nums)
            if pair_key in parallel_pairs:
                print(
                    f"[WARNING] parse_drop_target: '{direction_str}' 은 {mode} 모드에서 "
                    f"평행한 두 면의 조합입니다 (면 {nums[0]}과 면 {nums[1]}은 서로 마주보는 막리). "
                    f"모서리를 형성할 수 없으므로 Bottom 낙하로 fallback정니합니다."
                )
        for n in nums: 
    ...
    if np.linalg.norm(vec) < 1e-6:
        # [WHTOOLS] 유효한 방향을 결정할 수 없는 경우 (e.g. 평행 면 조합 fallback)
        if mode == 'LTL': vec = np.array([0.0, -1.0, 0.0]) # Bottom (Face 3)
        else: vec = np.array([0.0, -1.0, 0.0]) # Bottom (ISTA Default)
        print(f"[WARNING] parse_drop_target: '{direction_str}' -> 방향 파싱 실패. Bottom 낙하 fallback 적용.")
