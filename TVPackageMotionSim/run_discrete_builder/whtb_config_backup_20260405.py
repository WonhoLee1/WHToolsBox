...
def get_default_config(user_config: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    # -----------------------------------------------------------------
    # [WHTOOLS: V4.5 Update] ISTA 6-Amazon LTL 스펙을 기본값으로 상향
    # drop_mode: 'PARCEL' (택배 낙하), 'LTL' (대형 화물), 'CUSTOM' 등
    drop_mode = user_config.get("drop_mode", "LTL")
    # drop_direction: 'front', 'bottom', 'top-left-corner' 등 (whtb_utils.parse_drop_target 참고)
    drop_direction = user_config.get("drop_direction", "Corner 2-3-5")
    
    # -----------------------------------------------------------------
    # [1] 시뮬레이션/솔버 기본 파라미터 (MuJoCo Solver Options)
    # -----------------------------------------------------------------
    sim_integrator = user_config.get("sim_integrator", "implicitfast") # 솔버 알고리즘 (안정성 위주)
    sim_timestep   = user_config.get("sim_timestep", 0.0012)          # 시뮬레이션 타임스텝 (초)
    sim_iterations = user_config.get("sim_iterations", 50)            # 최대 컨택 반복 횟수
    sim_noslip_iterations = user_config.get("sim_noslip_iterations", 0) # 슬립 방지용 반복 횟수
    sim_tolerance  = user_config.get("sim_tolerance", 1e-5)           # 솔버 수렴 오차 한계
    ...
    cush_solref_timec = user_config.get("cush_solref_timec", 0.008) # 완충재 유연성 (0.008: 고정밀 LTL 튜닝값)
    ...
    # Ground (바닥 접촉면)
    ground_solref_timec = user_config.get("ground_solref_timec", 0.002)
    ground_solref_damprr = user_config.get("ground_solref_damprr", 0.001)
    ...
    oc_w_s = user_config.get('opencell_weld_solref_timec', user_config.get('cell_weld_solref_timec', 0.005))
    oc_w_d = user_config.get('opencell_weld_solref_damprr', user_config.get('cell_weld_solref_damprr', 0.5))
    cell_weld_solref = f"{oc_w_s} {oc_w_d}"
    
    ch_w_s = user_config.get('chassis_weld_solref_timec', user_config.get('tv_weld_solref_timec', 0.002))
    ch_w_d = user_config.get('chassis_weld_solref_damprr', user_config.get('tv_weld_solref_damprr', 0.3))
    ...
    # -----------------------------------------------------------------
    # 외부 박스 치수
    box_w = user_config.get("box_w", 1.841) 
    box_h = user_config.get("box_h", 1.103)
    box_d = user_config.get("box_d", 0.170)
    box_thick = user_config.get("box_thick", 0.008) # 박스 골판지 두께
    ...
    assy_w = user_config.get("assy_w", 1.670)
    assy_h = user_config.get("assy_h", 0.960)
    ...
    config = {
        ...
        "include_paperbox": user_config.get("include_paperbox", False),
        "include_cushion": True,
        "sim_duration": user_config.get("sim_duration", 2.2),
        
        "box_w": box_w, "box_h": box_h, "box_d": box_d,
        "box_thick": box_thick, "cush_gap": 0.005,
        ...
        "ground_solimp": user_config.get("ground_solimp", "0.1 0.95 0.002 0.5 2"),
        "ground_friction": user_config.get("ground_friction", 1.2), 
        ...
        "reporting_interval": user_config.get("reporting_interval", 0.0024), # 데이터 저장 주기 (초)
