...
class DropSimulator:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        Args:
            config (Dict, optional): 사용자 정의 시뮬레이션 파라미터. 
                전달되지 않으면 'run_discrete_builder' 패키지의 기본값을 사용합니다.
        """
        ...
        self.post_ui = None  # [v4] 포스트 프로세싱 Explorer (v4) 객체
    ...
    def _init_histories(self):
        ...
        self.structural_time_series = {
            ...
            'gti_max': [],          # [v4.1] Global Torsional Index (Max across components)
            'gbi_max': [],          # [v4.1] Global Bending Index (Max across components)
            'comp_global_metrics': {} # [v4.1] Component-specific global metrics histories
        }
        self.qpos_hist = []
        self.qvel_hist = []
    ...
    def _initialize_tracking_containers(self):
        ...
        for comp in self.components:
            self.metrics[comp] = {
                ...
                'all_blocks_bend_x'  : {idx: [] for idx in self.components[comp]}, # [v4.2] Spanwise Bending
                'all_blocks_bend_y'  : {idx: [] for idx in self.components[comp]}, # [v4.2] Ribwise Bending
                'all_blocks_twist'   : {idx: [] for idx in self.components[comp]},
                'max_rrg_hist'       : [], # [v4.2] 컴포넌트 내부 최대 RRG (부품별 비교용)
                'max_pba_hist'       : [], # [v4.2] 컴포넌트 내부 최대 PBA (부품별 비교용)
        ...
        # [COORD SYNC] Legacy Std: X(L/R) -> Y(B/T) -> Z(R/F)
        if 'bbox' in self.components:
            ...
            for coord, body_id in self.components['bbox'].items():
                if (coord[0] in (0, nx_max)) and (coord[1] in (0, ny_max)) and (coord[2] in (0, nz_max)):
                    corners.append((coord, body_id))
            
            # (i, j, k) 순서로 정렬하여 L-B-R, L-B-F, L-T-R, L-T-F, R-B-R, R-B-F, R-T-R, R-T-F 순서 보장
            corners.sort(key=lambda x: x[0])
            self.corner_body_ids = [c[1] for c in corners]
            self.log(f"  - Corner alignment complete (L-B-R to R-T-F): {len(self.corner_body_ids)} corners tracked.")
    ...
    def _apply_plasticity(self):
        ...
        try:
            ...
            # [6] 시각적 피드백 실시간 연동 (영구 변형량 비례 파란색 그라데이션)
            for gid, state in self.geom_state_tracker.items():
                axis_idx = state['major_axis']
                orig_sz = self.original_geom_size[gid][axis_idx]
                curr_sz = self.model.geom_size[gid][axis_idx]
                plastic_mm = (orig_sz - curr_sz) * 1000.0
                
                # 미세 오차 초과 시 영구 변형으로 간주하여 발색 적용
                if plastic_mm > 0.05: 
                    ...
                    if curr_color == list(self.original_geom_rgba[gid]) or (curr_color[0] == curr_color[1] and curr_color[2] > 0):
                        self.model.geom_rgba[gid] = [r_p, g_p, b_p, 1.0]
    ...
    def _run_simulation_instance(self):
        ...
        if self.config.get("use_viewer", False):
            ...
            # [v4.1] MuJoCo Viewer Window Size 설정 (프로그램 시작 혹은 설정값 반영)
            w, h = self.config.get('viewer_width', 1200), self.config.get('viewer_height', 900)
            os.environ["MJ_VIEWER_WIDTH"] = str(w)
            os.environ["MJ_VIEWER_HEIGHT"] = str(h)
            os.environ["MUJOCO_VIEWER_WIDTH"] = str(w)
            os.environ["MUJOCO_VIEWER_HEIGHT"] = str(h)
            ...
            with mujoco.viewer.launch_passive(self.model, self.data, key_callback=self._on_key) as viewer:
                ...
                # [v4] 시뮬레이션 시작 전 UI를 미리 실행하여 애니메이션/제어 활성화 (비활성화: 유저 요청)
                # if self.config.get("start_ui_before_sim", False):
                #     # [v4.2] 유저 요청: Viewer가 완전히 로딩된 후 UI가 등장해야 인식 가능
                #     time.sleep(1.0)
                #     from postprocess_ui import PostProcessingUI
                #     # 기존에 열려있는 UI가 있다면 닫고 새로 생성
                #     if self.post_ui and self.post_ui.winfo_exists():
                #         self.post_ui.destroy()
                #     self.post_ui = PostProcessingUI(self)
                #     self.post_ui.lift()
                #     self.post_ui.focus_force()
                #     self.tk_root.update()
                #     self.log(">> [UI] Post-Processing Explorer v4를 실행합니다.")
                ...
                if not (self.ctrl_quit_request or self.ctrl_reload_request):
                    ...
                    # 완료 후 UI가 닫혀있다면 명시적으로 한 번 더 노출
                    if getattr(self, "post_ui", None) and self.post_ui.winfo_exists():
                        try:
                            self.post_ui.on_simulation_complete()
                        except:
                            pass
                    else:
                        from postprocess_ui import PostProcessingUI
    ...
    def _main_loop(self, total_steps):
        ...
        try:
            while current_step_cnt < total_steps:
                ...
                # [v4] Post-Processing Explorer 실시간 동기화 (제거됨: 시뮬레이션 효율화)
                # if self.post_ui and self.post_ui.winfo_exists():
                #     if current_step_cnt % 5 == 0: 
                #         try:
                #             self.post_ui.update_live_data()
                #         except Exception as e:
                #             self.log(f"⚠️ [UI-Error] Real-time Update Failed: {e}")
            ...
            if current_step_cnt >= total_steps:
                ...
                while self.viewer and self.viewer.is_running():
                    self.tk_root.update()
                    if self.ctrl_reset_request:
                        self.log("\n>> [Simulator] 리셋 요청 수신. 시물레이션을 처음부터 다시 시작합니다.")
                        self._reset_simulation_state()
                        current_step_cnt = 0
                        self.state_buffer.clear()
                        self._print_terminal_header()
                        # 내부 루프를 깨고 시뮬레이션 메인 루프(while current_step_cnt < total_steps)로 이동
                        break 
                    
                    if self.ctrl_quit_request or self.ctrl_reload_request:
                        return # 메인 루프 종료 (simulate()에서 reload 처리됨)
                        
                    self.viewer.sync()
                    time.sleep(0.01)
                
                # 리셋으로 인해 루프를 탈출한 경우, 상위 시뮬레이션 루프를 다시 호출하여 처음부터 시작 (Tail Recursion)
                if self.ctrl_reset_request:
                    self.ctrl_reset_request = False
                    self._main_loop(total_steps)
                    return
    ...
    def _collect_step_data(self):
        ...
        # 전체 QPOS/QVEL 히스토리 저장 (Post-UI 동기화용)
        ...
        if root_id != -1:
            ...
            # [v4] 트래킹 대상 바디가 없거나 부족한 경우, 유도된 기구학적 위치값을 사용 (변위 0 출력 방지)
            if not c_pos_now:
                c_pos_now = [ck['pos'] for ck in corner_kin]
        ...
        if len(self.time_history) % 5 != 1:
            ...
            for ts_key, ts_list in self.structural_time_series.items():
                if isinstance(ts_list, list):
                    last_v = ts_list[-1] if ts_list else (0.0 if 'vector' not in ts_key else np.zeros(3))
                    ts_list.append(last_v)
                elif ts_key == 'comp_global_metrics' and isinstance(ts_list, dict):
                    # 각 컴포넌트별 리스트를 순회하며 복제
                    for comp_name in ts_list:
                        for m_key in ['gti', 'gbi']:
                            h_list = ts_list[comp_name][m_key]
                            if isinstance(h_list, list):
                                last_v = h_list[-1] if h_list else 0.0
                                h_list.append(last_v)
            return
        ...
        for comp_name, comp_metric in self.metrics.items():
            ...
            for grid_idx, body_uid in self.components[comp_name].items():
                ...
                # [v4.2] 성분별 벤딩 데이터 수집 (로컬 X/Y축 기반)
                # rot_vec의 X성분은 YZ평면에서의 휨, Y성분은 XZ평면에서의 휨에 해당
                comp_metric['all_blocks_bend_x'][grid_idx].append(np.degrees(rot_vec[0]))
                comp_metric['all_blocks_bend_y'][grid_idx].append(np.degrees(rot_vec[1]))
                
                # PBA용 글로벌 수집 (XY 평면 성분만: 벤딩축에 해당)
                global_rot_vectors.append(rot_vec)
                global_rot_positions.append(grid_idx[:2])  # (i, j) 좌표
            
            # [v4.2] PBA 부품별 요약 데이터 추출
            # 여기서는 편의상 해당 부품 블록들 중 최대 각도를 사용
            pba_max_in_comp = max(list_of_angles) if list_of_angles else 0.0
            comp_metric['max_pba_hist'].append(pba_max_in_comp)
            ...
            if comp_name in self.neighbor_map:
                ...
                # [v4.2] RRG 부품별 요약 저장
                max_rrg_in_comp = max((comp_metric['all_blocks_rrg'][idx][-1] for idx in self.components[comp_name] if comp_metric['all_blocks_rrg'][idx]), default=0.0)
                comp_metric['max_rrg_hist'].append(max_rrg_in_comp)
        ...
        # [v4.1] Global Metrics (GTI, GBI)
        step_gti_max = 0.0
        step_gbi_max = 0.0
        
        for comp_name, comp_metric in self.metrics.items():
            if comp_name not in self.structural_time_series['comp_global_metrics']:
                self.structural_time_series['comp_global_metrics'][comp_name] = {'gti': [], 'gbi': []}
            
            # --- GTI (Global Torsional Index) ---
            # 장축 판단
            idxs = list(self.components[comp_name].keys())
            if not idxs: continue
            max_idx = np.max(idxs, axis=0)
            min_idx = np.min(idxs, axis=0)
            ranges = max_idx - min_idx
            major_dim = np.argmax(ranges)
            
            # 양 끝단 블록 식별
            end_a_idxs = [idx for idx in idxs if idx[major_dim] == min_idx[major_dim]]
            end_b_idxs = [idx for idx in idxs if idx[major_dim] == max_idx[major_dim]]
            
            # 끝단별 평균 Twist 추출 (v4.1)
            twist_a = np.mean([comp_metric['all_blocks_twist'][idx][-1] for idx in end_a_idxs]) if end_a_idxs else 0.0
            twist_b = np.mean([comp_metric['all_blocks_twist'][idx][-1] for idx in end_b_idxs]) if end_b_idxs else 0.0
            
            # GTI: 방향성 비틀림 차이 (교수님 정의 반영)
            gti_val = abs(twist_a - twist_b)
            self.structural_time_series['comp_global_metrics'][comp_name]['gti'].append(gti_val)
            step_gti_max = max(step_gti_max, gti_val)
            
            # --- GBI (Global Bending Index) ---
            # 전체 블록의 벤딩(Tilt) 평균 또는 중심선 곡률
            bend_vals = [comp_metric['all_blocks_bend'][idx][-1] for idx in idxs]
            gbi_val = np.max(bend_vals) if bend_vals else 0.0
            self.structural_time_series['comp_global_metrics'][comp_name]['gbi'].append(gbi_val)
            step_gbi_max = max(step_gbi_max, gbi_val)
        ...
        self.structural_time_series['gti_max'].append(step_gti_max)
        self.structural_time_series['gbi_max'].append(step_gbi_max)
    ...
    def _finalize_simulation(self):
        ...
        # [NEW v11] 시뮬레이션 결과(Result Container) 객체 갱신 및 바이너리 데이터화
        # 이 변수가 할당되어야 plot_results() 및 외부 툴에서 데이터를 인식할 수 있습니다.
        t_hist = self.time_history
        acc_np = np.array(self.acc_hist)
        root_acc_z = acc_np[:, 5] if acc_np.ndim >= 2 and acc_np.shape[1] > 5 else (acc_np[:, 2] if acc_np.ndim >= 2 else np.zeros(len(t_hist)))
        ct = {}
        if sts.get('pba_magnitude'):
            pba_m_idx = int(np.argmax(sts['pba_magnitude']))
            ct['pba_peak_time'] = float(t_hist[pba_m_idx]) if pba_m_idx < len(t_hist) else 0.0
            ct['pba_peak_angle'] = float(sts['pba_angle'][pba_m_idx]) if pba_m_idx < len(sts['pba_angle']) else 0.0

        if sts.get('rrg_max'):
            rrg_m_idx = int(np.argmax(sts['rrg_max']))
            ct['local_peak_time'] = float(t_hist[rrg_m_idx]) if rrg_m_idx < len(t_hist) else 0.0
            ct['local_peak_rrg'] = float(sts['rrg_max'][rrg_m_idx])

        if sts.get('mean_distortion'):
            mean_m_idx = int(np.argmax(sts['mean_distortion']))
            ct['global_avg_peak_time'] = float(t_hist[mean_m_idx]) if mean_m_idx < len(t_hist) else 0.0
            ct['global_avg_peak_val'] = float(sts['mean_distortion'][mean_m_idx])

        self.result = DropSimResult(
            config=self.config.copy(),
            metrics=self.metrics.copy(),
            max_g_force=float(np.max(np.abs(root_acc_z)) / 9.81) if len(root_acc_z) > 0 else 0.0,
            time_history=t_hist.copy(),
            z_hist=self.z_hist.copy(),
            root_acc_history=root_acc_z.tolist(),
            corner_acc_hist=self.corner_acc_hist.copy(),
            pos_hist=self.pos_hist.copy(),
            vel_hist=self.vel_hist.copy(),
            acc_hist=self.acc_hist.copy(),
            cog_pos_hist=self.cog_pos_hist.copy(),
            cog_vel_hist=self.cog_vel_hist.copy(),
            cog_acc_hist=self.cog_acc_hist.copy(),
            geo_center_pos_hist=self.geo_center_pos_hist.copy(),
            geo_center_vel_hist=self.geo_center_vel_hist.copy(),
            geo_center_acc_hist=self.geo_center_acc_hist.copy(),
            corner_pos_hist=self.corner_pos_hist.copy(),
            corner_vel_hist=self.corner_vel_hist.copy(),
            ground_impact_hist=self.ground_impact_hist.copy(),
            air_drag_hist=self.air_drag_hist.copy(),
            air_viscous_hist=self.air_viscous_hist.copy(),
            air_squeeze_hist=self.air_squeeze_hist.copy(),
            structural_metrics=self.structural_time_series.copy(),
            critical_timestamps=ct
        )
    ...
    def open_post_ui(self):
        """시뮬레이션 완료 후 포스트 프로세싱 UI를 엽니다. 이미 열려있다면 데이터만 갱신합니다."""
        if not self.tk_root: return
        
        if self.post_ui and self.post_ui.winfo_exists():
            # [v4.2] 기존 터미널 UI와 연동: 이미 열려 있다면 데이터 갱신 메서드 호출
            if hasattr(self.post_ui, 'on_simulation_complete'):
                # [v4.3] 대기 루틴 충돌 방지를 위해 비동기 호출 (after)
                self.tk_root.after(100, self.post_ui.on_simulation_complete)
            self.post_ui.lift()
            self.post_ui.focus_force()
        else:
            from postprocess_ui import PostProcessingUI
    ...
    def set_state_at(self, step_idx: int):
        """[v4.2] 지정된 타임스텝의 상태로 MuJoCo 모델을 강제 복원합니다 (Post-UI 연동용)"""
        if not hasattr(self, 'qpos_hist') or not self.qpos_hist:
            return
        if step_idx < 0 or step_idx >= len(self.qpos_hist):
            return
            
        self.data.qpos[:] = self.qpos_hist[step_idx]
        self.data.qvel[:] = self.qvel_hist[step_idx]
