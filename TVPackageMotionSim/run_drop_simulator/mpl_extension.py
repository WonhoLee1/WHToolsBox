...
class WHToolsPlotManager:
    ...
    def _setup_tk_menu(self, fig):
        ...
        try:
            import tkinter as tk
            if not hasattr(fig.canvas.manager, 'window') or fig.canvas.manager.window is None:
                return
    ...
    def _on_hover(self, event, fig):
        """마우스 호버 시 가장 가까운 점에 대한 안내 레이블 표시 (다중 서브플롯 지원)"""
        if fig not in self._hover_labels:
            self._hover_labels[fig] = {}

        if not event.inaxes:
            for ax_label in self._hover_labels[fig].values():
                if ax_label: ax_label.set_visible(False)
            fig.canvas.draw_idle()
            return

        ax = event.inaxes
        for other_ax, other_label in self._hover_labels[fig].items():
            if other_ax != ax and other_label:
        ...
        if min_dist < 20 and closest_point:
            if ax not in self._hover_labels[fig]:
                self._hover_labels[fig][ax] = ax.annotate(
                    "", xy=(0,0), xytext=(20, 20), textcoords="offset points",
                    bbox=dict(boxstyle="round", fc="w", alpha=0.8),
                    arrowprops=dict(arrowstyle="->"))
            
            ann = self._hover_labels[fig][ax]
            ann.set_visible(True)
            ann.xy = closest_point
            ann.set_text(f"x: {closest_point[0]:.4f}\ny: {closest_point[1]:.4f}")
            fig.canvas.draw_idle()
        else:
            if ax in self._hover_labels[fig]:
                self._hover_labels[fig][ax].set_visible(False)
                fig.canvas.draw_idle()

    def _on_click(self, event, fig):
        """클릭 시 현재 호버링 중인 지점을 고정 마킹 (다중 서브플롯 지원)"""
        if event.button == 1 and event.inaxes:
            ax = event.inaxes
            if fig in self._hover_labels and ax in self._hover_labels[fig]:
                ann_guide = self._hover_labels[fig][ax]
