...
def get_face_index_logic(face_name: str, max_indices: Tuple[int, int, int]) -> Dict[str, Any]:
    """
    [WHTOOLS Standard Coordinate Mapping]
    명칭별 격자 인덱스(i,j,k) 및 면 법선 방향(normal)을 정의합니다.
    - X: Width (Left/Right)
    - Y: Depth (Front/Rear)
    - Z: Height (Top/Bottom) - MuJoCo Standard Z-Up
    """
    max_i, max_j, max_k = max_indices
    logic = {
        "Top":    {"axis": "k", "idx": max_k, "normal": [0,0,1],  "plane": ("i","j"), "offsets": ["dx","dy"]},
        "Bottom": {"axis": "k", "idx": 0,     "normal": [0,0,-1], "plane": ("i","j"), "offsets": ["dx","dy"]},
        "Front":  {"axis": "j", "idx": max_j, "normal": [0,1,0],  "plane": ("i","k"), "offsets": ["dx","dz"]},
        "Rear":   {"axis": "j", "idx": 0,     "normal": [0,-1,0], "plane": ("i","k"), "offsets": ["dx","dz"]},
...
def extract_face_markers(result: DropSimResult, component_name: str) -> Tuple[Dict[str, np.ndarray], Dict[str, np.ndarray]]:
    ...
    for face_name in face_prefix:
        ...
        v_sign = -1.0 if face_name == "Bottom" else 1.0
