...
class QtVisualizerV2(QtWidgets.QMainWindow):
    ...
    def _init_animation_toolbar(self):
        tb = self.addToolBar("Animation Control"); tb.setFixedHeight(40)
    ...
    def _init_3d_view(self):
        ...
        for i, p in enumerate(self.mgr.analyzers):
            poly = pv.Plane(i_size=p.W, j_size=p.H, i_resolution=p.sol.res-1, j_resolution=p.sol.res-1); ma = self.v_int.add_mesh(poly, scalars=np.zeros(p.sol.res**2), cmap=self.lut, show_edges=True, edge_color="darkgray", show_scalar_bar=False)
