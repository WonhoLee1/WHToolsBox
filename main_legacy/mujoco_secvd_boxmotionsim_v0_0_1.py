...
def plot_joint_doe_comparison(sim):
    ...
    col_titles = ["Overall Avg Rotation", "X-Joint Avg Rotation", "Y-Joint Avg Rotation"]
    colors = ['#e74c3c', '#3498db', '#2ecc71', '#f1c40f', '#9b59b6'] 
    offset_labels = ["-50mm", "-25mm", "0mm", "+25mm", "+50mm"]

    for r in range(3): # Sweep Axis (X, Y, Z)
        for c_type in range(3): # Plot Type (Overall, X, Y)
            ax = axes[r, c_type]
            ax.set_title(f"{sweep_axis_names[r]}\n{col_titles[c_type]}", fontsize=11, fontweight='bold')
            ...
            for c_offset in range(5): # Individual Case in Sweep
                ...
                # Filter joints by type
                targets = []
                if c_type == 0: # Overall
                    targets = list(j_angles.values())
    ...
    for i in range(3): axes[2, i].set_xlabel("Time (seconds)")
...
def plot_peak_joint_doe_comparison(sim):
    ...
    axes[2].set_xlabel("Time (seconds)")
    fig.suptitle(f"DOE Worst-Case Joint Rotation (K={instances[0].flex_stiffness}, D={instances[0].flex_damping})", fontsize=15, fontweight='bold')
    plt.tight_layout(rect=[0, 0.03, 1, 0.97])
    plt.show()
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if is_sweep_case:
            ...
            print(f"  [98] View DOE Joint Rotation Comparison (3x3 Grid)")
