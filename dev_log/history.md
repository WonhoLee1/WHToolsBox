# 2026-04-30
MuJoCo 시뮬레이션 에디터 및 물리 파이프라인 최적화 작업을 수행하였다.
- **실시간 XML 에디터 구현**: `whts_control_panel.py`에 `XMLEditorDialog`를 추가하여 시뮬레이션 도중 XML 수식을 직접 수정하고 즉시 반영(Apply & Reload)할 수 있는 기능을 구현하였다. 외부 에디터(VS Code 등) 연동 기능도 포함되었다.
- **물리 엔진 최적화**: `whts_engine.py`의 공기역학 및 소성 변형 로직을 Numba JIT 및 NumPy 벡터화 방식으로 재구축하여 연산 속도를 대폭 향상시켰다.
- **시각화 및 안정성 개선**: MuJoCo 뷰어 창의 위치를 자동으로 감지하여 제어 패널을 우측 상단에 정렬하는 기능을 추가하였으며, `find_mujoco_window.py` 유틸리티를 통해 윈도우 핸들 관리의 안정성을 높였다.
- **인터랙티브 기능 확장**: 카메라 시점 프리셋(X, Y, Z, ISO 등) 버튼과 실시간 모션 로그 기능을 추가하여 분석 편의성을 개선하였다.

# 2026-03-24
cushion의 plastic defomation을 구현하는 방법으로 floor와의 침투량을 측정하고 이것이 회복될 때 (침투량이 감소할 때) 그 양의 일부를 영구변형을 보는 알고리즘을 이전에 구현했었다.

이 알고리즘이 현재 작동되는 상황을 보면, 쿠션의 8곳의 꼭지점에서는 기본적으로 작동이 되어야 하는데, 현재 작동이 안되는 것으로 보인다. 작동의 여부는 floor와의 충돌 후에 약간의 침투는 접촉 알고리즘 상 발생을 할 것이고 일부가 영구변형 처리가 되기 때문에 이를 통해 확인한 것이다. 그런데 특이 한 것은 쿠션의 두께 (또는로컬 z방향)에서 꼭지점 중간에 속하는 geom에서는 이 변화가 있었다. 

따라서, 이 알고리즘이 작동되어야 할 쿠션을 꼭지점과 모서리로 했었는데, 이 과정에서 꼭지점에 위치하는 geom 쿠션블럭이 포함되지 않았을 가능성을 생각해봤다.

플라스틱이 작동되는 geom을 찾으면 이 geom의 색상은 옅은 노란색으로 해서 표시를 해보자. 그리고 플라스틱 변형 처리가 되면서 파란색으로 변경하도록 해보자.



# 2026-03-23

run_drop_simulation.py의 내용을 개선작업할 것이다.

cfg 설정 중 cush_solref, tape_solref, cell_solref, tv_solref 및 ~_solimp에 대한 설정이  weld에 대해서 적용되길 기대했는데, 이것이 BaseDiscreteBody을 파생하는 클래스에서 material_props로 적용되어 반영되길 기대했는데, 작동되지 않아 보인다.

이 수정 시에 기본 값으로 
<weld site1="s_BCushion_2_0_1_PZ" site2="s_BCushion_2_0_2_NZ" solref="0.1 2.0" solimp="0.95 0.999 0.001 0.5 2.0"/>
와 같이 solrelf, solimp가 적용이 될 것을 기대하는데,
단, default class를 이용할 수 있는지 궁금하다. 즉, 각 부품 클래스에 해당하는 weld 의 속성이므로 부품별로 일괄적으로 관리할 수 있게 하여 최상단 default xml에서 수정을 하면 해당 부품이 전체 적용되게 하자.

cushion과 지면과의 접촉에 대한 solref, solimp는 cfg에서 어떻게 설정할까? 바닥에 대한 것은 독자적으로 설정하고 (이것은 이미 있다.) 살펴보니, 접촉과 weld가 동일한 solref, solimp를 사용하는 것으로 오해할 수 있어 보여서이다. 

cushion의 경우 외곽 상자 기준 엣지, 모서리의 cushion만 별도의 접촉(지면과의)과 관련된 solref, solimp를 정의해야 할 것으로 보인다. 이에 대한 내용을 config에 추가하자. 그리고 main test 코드에 추가한다.

사용자께서 요청하신 run_drop_simulation.py 및 관련 클래스의 weld와 contact 파라미터 개선 작업을 위한 구현 계획을 수립했습니다. 주요 변경 사항은 다음과 같습니다:

Weld 및 Contact 파라미터 분리: 기존에 혼용되던 파라미터를 weld_solref와 contact_solref로 명확히 구분하여 부위별/용도별로 세밀하게 조정할 수 있도록 합니다.
부품별 기본값(Default) 관리: 최상단 config에서 부품별 weld 속성을 일괄적으로 관리할 수 있도록 구조를 개선합니다.
쿠션 엣지/모서리 특화 설정: 쿠션의 위치(엣지, 코너)를 자동으로 판별하여 지면 충돌 시 별도의 solref, solimp를 적용하는 로직을 추가합니다.


# 2026-03-06
현재 코드 개선 및 개발 과정에서 코드를 축약하지 말자!
주석도 풍부하게 진행하자!
구조적이고 체계적으로 작성하라! 재활용성이 높게 작성하라!

- cushion 속성 정의 개선
  모서리 부분의 cushion에 대해서 지면과의 충돌 시의 접촉 특성을 다른 cushion 구성 블럭들과 다르게 설정할 수 있게 config 추가한다.
  

1. 현재는 결과 추출을 위한 시뮬레이션 호출을 하고 있다.
   시작 시에 mujoco UI를 열고 이를 이용하는 기능과
   결과 추출을 위한 시뮬레이션 호출을 선택하는 기능을 구분하다.
   a. 나는 지금 mujoco UI에서 시뮬레이션을 확인하는 과정이다.
   b. 나는 UI없이 시뮬레이션하고 결과를 추출하는 과정이다.

2. UI 시뮬레이션에서 config 파라미터를 UI로 조정할 수 있는 기능이 있으면 좋겠다.
   config 파라미터는 대다수 key, value로 되어 있으니까,
   특정 버튼을 누르면, UI로 key, value를 변경할 수 있도록 나타나고, 
   이를 근거로 다시 모델을 생성해서 reload하는 기능이다.
   그러기 위해서 UI가 체계적이어야 할 것 같고, 트리 구조나 리스트 구조로 많은 파라미터를 선택하고 수정할 수 있게 효과적인 UI를 개발한다. 
   
   PySide를 이용하면 좋을 것 같은데, mujoco와 연동이 잘 되어야 할 것 같다.
   
   여기에는 낙하 자세를 변경하는 기능도 포함한다. 

   이 경우는 xml 재생성과 reload를 해야하는 것으로 생각한다.

   그런데, xml 재생성없이 바로 UI에서 파라미터를 변경하고 시뮬레이션을 다시 실행하는 기능이 있으면 좋겠다.
   어떤 파라미터는 가능하고 어떤 것은 안될 것 같기는 하다.
   weld soft 지정 파라미터는 공통으로 변경할 수 있는 항목 같은데,
   이 경우는 geom은 많이 생기지만, 값 변경은 한번에 할 수 있게 class 태그로 관리할 수 있지 않나?

  <geom name="g_bcushion_0_0_2" type="box" size="0.07450 0.07450 0.03825" mass="0.008850" rgba="0.9 0.9 0.9 0.5" contype="2" conaffinity="29" group="1" friction="0.8" solref="0.07 0.7" solimp="0.95 0.999 0.001 0.5 2.0"/>

  이 경우라면, solimp, solref, friction 등은 common class로 공통화 할 수 있지 않나?
